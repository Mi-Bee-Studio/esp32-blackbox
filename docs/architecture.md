...
- **/probe 端点**: 支持实时执行、即席探测和 blackbox_exporter 风格的抓取配置
- **即席探测**: 支持任意主机名/IP 地址，可选端口参数
### 2. 模块化探测系统
...
### 5. 统一 Web 服务器 (支持 AP + STA + 即席探测)

统一的 Web 服务器，提供三种模式的服务：
...
**即席探测模式 (动态执行):**
- `/probe?target=X&module=Y&port=P` 实时探测
- 支持任意主机名和 IP 地址
- 返回 Prometheus 格式的即时结果
...
if (wifi_manager_is_connected()) {
    // STA 模式 - 启动配置管理 API 和即席探测
ST    web_server_handle_probe_request();    // /probe 端点
JN    web_server_start_config_api();       // /api/* 端点
YM} else {
QJ    // AP 模式 - WiFi 配置门户
YM    wifi_config_server_start();         // /scan, /save 端点
PB}
```
  │
...
#### /probe 端点 (实时执行)
```
#MVGET /probe?target=httpbin.org&module=http_2xx
#NHHTTP/1.1 200 OK
#HVContent-Type: text/plain; version=0.0.4
...
#SRprobe_duration_seconds{target="httpbin.org", module="http_2xx"} 0.234
 ...
 HELP probe_success Whether the probe succeeded
 TYPE probe_success gauge
#KKprobe_success{target="httpbin.org", module="http_2xx"} 1

 HELP probe_status_code Status code or result code
 TYPE probe_status_code gauge
#KQprobe_status_code{target="httpbin.org", module="http_2xx"} 200
### 配置管理 API
...
- [x] ICMP ping 探测
- [x] 热加载配置机制
- [x] 统一 Web 服务器 (AP + STA + 即席探测)
- [x] /probe 端点实时执行
- [x] 黑盒导出器兼容标签体系
- [x] 即席探测功能 - 支持任意主机/IP 和端口参数
